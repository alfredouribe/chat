...
class User extends Authenticatable
{
    ...
    public static function getUsersExceptUser(User $exceptUser){
        ...
        ->when(!$exceptUser->id_admin, function($query){
        ...
        ->orderByRaw('IFNULL(users.blocked_at, 1)')//send blocked users to the bottom
        ->orderBy('messages.created_at', 'desc')//show first the users who sent the last messages
        ->orderBy('users.name')//if there is no conversations then orders by name
        ;
